...
1. **Python Analysis Engine** (`engine/`) — The backend that runs security scans
...
|---|---|---|
| `config.py` | All configuration (SonarQube, LLM, datasets, evaluation) | `SonarQubeConfig`, `LLMConfig`, `DatasetConfig`, `EvaluationConfig` |
| `dataset_manager.py` | Load/export vulnerability test samples | `DatasetManager`, `VulnerabilitySample`, `DatasetStats` |
| `sonarqube_scanner.py` | SonarQube REST API + simulated SAST detection | `SonarQubeScanner`, `SonarQubeIssue`, `SonarQubeScanResult` |
| `llm_analyzer.py` | Claude LLM vulnerability analysis with structured prompts | `LLMAnalyzer`, `LLMFinding`, `LLMAnalysisResult` |
| `evaluator.py` | Binary classification metrics + cost analysis | `DetectionMetrics`, `CostMetrics`, `ComparisonResult`, `Evaluator` |
| `run_comparison.py` | Main orchestrator — runs the full pipeline | `run_full_comparison()` |
...
```
src/data/comparison_report.json  (generated by Python engine)
       |
...
- `sonarqube_scanner.py` → `SIMULATED_RESULTS` dict
- `llm_analyzer.py` → `SIMULATED_RESULTS` dict (per model)
...
- Replace the `scan_sample()` / `analyze()` methods with actual API calls
...
1. Add sample data to `dataset_manager.py` → `SAMPLE_VULNERABILITIES[language]`
2. Add expected SonarQube detection to `sonarqube_scanner.py` → `SIMULATED_RESULTS`
3. Add expected LLM detection to `llm_analyzer.py` → `SIMULATED_RESULTS` (for each model)
4. Re-run `python engine/run_comparison.py`

### Adding a New LLM Model

1. Add model config to `config.py` → `LLMConfig.pricing`
2. Add token estimates to `llm_analyzer.py` → `AVG_TOKENS_PER_SAMPLE`
3. Add simulated results to `llm_analyzer.py` → `SIMULATED_RESULTS[new_model_id]`
4. Add model to `run_comparison.py` analysis pipeline
...
### Backend
- Python 3.10+
- `requests` (SonarQube API calls)
- Standard library only (no heavy dependencies by design)
...
3. **No async/await** — The engine uses synchronous patterns. For production with real APIs, refactor to async.
